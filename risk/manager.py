# ============================================================
#  risk/manager.py
#  Fix #11: alert when balance is zero
#  Fix #7:  is_daily_limit_hit now tracked and enforced
# ============================================================
import os
import json
...
class RiskManager:
    ...
    def calculate_position_size(self, balance: float, entry_price: float,
        ...
        """
        Fix #11: returns -1 if balance is zero so caller can alert.
        Otherwise returns quantity to buy.
        """
        if balance <= 0:
            return -1.0   # Fix #11: signal to caller that balance is missing
    ...
    def is_daily_limit_hit(self, balance: float) -> tuple[bool, str]:
        """
        Fix #7: actually checks and enforces the daily loss limit.
        Returns (halted, message).
        """
        data  = self._load_daily()
        if data.get("halted"):
