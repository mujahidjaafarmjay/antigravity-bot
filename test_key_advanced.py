...
print("Original length:", len(b64_payload))

import string
clean_payload = "".join(c for c in b64_payload if c in string.ascii_letters + string.digits + "+/=")
print("Cleaned length:", len(clean_payload))

if len(clean_payload) % 4 != 0:
    print(f"Padding missing or extra characters! Length mod 4 = {len(clean_payload) % 4}")
    # Let's try to remove trailing '=' and re-pad
    clean_payload = clean_payload.rstrip('=')
    clean_payload += '=' * (-len(clean_payload) % 4)
    print("Repadded length:", len(clean_payload))

try:
    base64.b64decode(clean_payload, validate=True)
    print("Success after cleaning!")
except Exception as e:
    print("Still failed:", e)

    # Let's see if standard cryptography loading works
from cryptography.hazmat.primitives import serialization
from cryptography.hazmat.backends import default_backend

# Test 1: Let's see if the key can be loaded despite the issue?
fixed_pk = "-----BEGIN PRIVATE KEY-----\n" + "\n".join(clean_payload[i:i+64] for i in range(0, len(clean_payload), 64)) + "\n-----END PRIVATE KEY-----\n"

try:
    serialization.load_pem_private_key(fixed_pk.encode("utf-8"), password=None, backend=default_backend())
    print("Cryptography loaded the fixed key successfully!")
except Exception as e:
    print("Cryptography failed to load fixed key:", e)
